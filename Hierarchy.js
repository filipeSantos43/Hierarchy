...
var rightShoulderMatrix = new Matrix4()
    .setTranslate(-6.5, 2, 0)  // Notice the negative X value for the right side
    .translate(0, 2, 0)
    .rotate(-joint.rightshoulder, 1, 0, 0)
    .translate(0, -2, 0);

var rightArmMatrix = new Matrix4()
    .setTranslate(0, -5, 0)
    .translate(0, 2.5, 1.0)
    .rotate(-joint.rightarm, 1, 0, 0)
    .translate(0, -2.5, -1.0);

var rightHandMatrix = new Matrix4()
    .setTranslate(0, -4, 0)
    .rotate(joint.righthand, 0, 1, 0);
    
var rightThighMatrix = new Matrix4()
    .setTranslate(-6.5, -5, 0)  // Notice the negative X value for the right side
    .translate(0, 2, 0)
    .rotate(-joint.rightThight, 1, 0, 0)
    .translate(0, -2, 0);
    
var rightCalfMatrix = new Matrix4()
    .setTranslate(0, -5, 0)  // Notice the negative X value for the right side
    .translate(0, 2, 0)
    .rotate(-joint.rightCalf, 1, 0, 0)
    .translate(0, -2, 0);
    
var rightFootMatrix = new Matrix4()
    .setTranslate(0, -4, 0)  // Notice the negative X value for the right side
    .translate(0, 2, 0)
    .rotate(-joint.rightFoot, 1, 0, 0)
    .translate(0, -2, 0);



/**  @type {Matrix4} */
var leftThighMatrix = new Matrix4()
    .setTranslate(6.5, -5, 0)
    .translate(0, 2, 0)
    .rotate(-joint.leftThigh, 1, 0, 0)
    .translate(0, -2, 0);

/**  @type {Matrix4} */
var leftCalfMatrix = new Matrix4()
    .setTranslate(0, -5, 0)
    .translate(0, 2.5, 1.0)
    .rotate(-joint.leftCalf, 1, 0, 0)
    .translate(0, -2.5, -1.0);

/**  @type {Matrix4} */
var leftFootMatrix = new Matrix4()
    .setTranslate(0, -4, 0)
    .rotate(joint.leftFoot, 0, 1, 0);
...
function handleKeyPress(event) {
    ...
    switch (ch) {
        ...
        case "s":
            ...
            joint.rightshoulder += 15;
            // rotate shoulder clockwise about a point 2 units above its center
            var currentShoulderRot2 = new Matrix4()
                .setTranslate(0, 2, 0)
                .rotate(-joint.rightshoulder, 1, 0, 0)
                .translate(0, -2, 0);
            rightShoulderMatrix.setTranslate(-6.5, 2, 0).multiply(currentShoulderRot2);
            break;
        case "S":
            joint.shoulder += 15;
            // rotate shoulder clockwise about a point 2 units above its center
            ...
            break;
           case "p":
            joint.rightThight += 15;
            // rotate shoulder clockwise about a point 2 units above its center
            var currentThighRot = new Matrix4()
                .setTranslate(-6.5, -5, 0)
                .rotate(-joint.rightThight, 1, 0, 0)
                .translate(0, -2, 0);
            rightThighMatrix.setTranslate(0, 1.5, 0).multiply(currentThighRot);
            
            joint.leftThigh += 15;
            // rotate shoulder clockwise about a point 2 units above its center
            var currentThighRot2 = new Matrix4()
                .setTranslate(6.5, -5, 0)
                .rotate(-joint.leftThigh, 1, 0, 0)
                .translate(0, -2, 0);
            leftThighMatrix.setTranslate(0, 1.5, 0).multiply(currentThighRot2);
            break;
            case "P":
           joint.rightThight += 15;
            // rotate shoulder clockwise about a point 2 units above its center
        ...
        case "a":
            ...
            armMatrix.setTranslate(0, -5, 0).multiply(currentArm);
             ...
             joint.rightarm += 15;
            // rotate arm clockwise about its top front corner
            var currentArm2 = new Matrix4()
                .setTranslate(0, 2.5, 1.0)
                .rotate(-joint.rightarm, 1, 0, 0)
                .translate(0, -2.5, -1.0);
            rightArmMatrix.setTranslate(0, -5, 0).multiply(currentArm2);
            break;
        case "A":
             joint.arm += 15;
            // rotate arm clockwise about its top front corner
           ...
           case "c":
             joint.rightCalf += 15;
            // rotate arm clockwise about its top front corner
            var currentCalf = new Matrix4()
                .setTranslate(0, 2.5, 1.0)
                .rotate(-joint.rightCalf, 1, 0, 0)
                .translate(0, -2.5, -1.0);
            rightCalfMatrix.setTranslate(0, -5, 0).multiply(currentCalf);
            
             joint.leftCalf += 15;
            // rotate arm clockwise about its top front corner
            var currentCalf2 = new Matrix4()
                .setTranslate(0, 2.5, 1.0)
                .rotate(-joint.leftCalf, 1, 0, 0)
                .translate(0, -2.5, -1.0);
            leftCalfMatrix.setTranslate(0, -5, 0).multiply(currentCalf2);
            break;
             case "C":
             joint.rightCalf += 15;
            // rotate arm clockwise about its top front corner
        ...
        case "h":
            ...
            joint.righthand += 15;
            rightHandMatrix.setTranslate(0, -4, 0).rotate(joint.righthand, 0, 1, 0);
            break;
        case "H":
            joint.hand -= 15;
            handMatrix.setTranslate(0, -4, 0).rotate(joint.hand, 0, 1, 0);
           ...
           case "f":
            joint.rightFoot += 15;
            rightFootMatrix.setTranslate(0, -4, 0).rotate(joint.rightFoot, 0, 1, 0);
            
            joint.leftFoot += 15;
            leftFootMatrix.setTranslate(0, -4, 0).rotate(joint.leftFoot, 0, 1, 0);
            break;
              case "F":
            joint.rightFoot += 15;
            rightFootMatrix.setTranslate(0, -4, 0).rotate(joint.rightFoot, 0, 1, 0);
...
function draw(useRotator = true) {
   ...
   // Render the left arm
    ...
    // Pop the matrices for the left arm to get back to the torso matrix
    s.pop(); // hand
    s.pop(); // arm
    s.pop(); // shoulder
    ...
    renderCube(s, shoulderMatrixLocal); // Using the same local matrix for both arms

    s.push(new Matrix4(s.top()).multiply(rightArmMatrix));
    renderCube(s, armMatrixLocal); // Using the same local matrix for both arms

    s.push(new Matrix4(s.top()).multiply(rightHandMatrix));
    renderCube(s, handMatrixLocal); // Using the same local matrix for both arms
    ...
    s.pop(); // shoulder
        ...
        // Render the right thigh
    s.push(new Matrix4(s.top()).multiply(rightThighMatrix));
    renderCube(s, shoulderMatrixLocal);
    
    s.push(new Matrix4(s.top()).multiply(rightCalfMatrix));
    renderCube(s, armMatrixLocal);
	...
	s.push(new Matrix4(s.top()).multiply(leftThighMatrix));
    renderCube(s, shoulderMatrixLocal);
    
    s.push(new Matrix4(s.top()).multiply(leftCalfMatrix));
    renderCube(s, armMatrixLocal);
